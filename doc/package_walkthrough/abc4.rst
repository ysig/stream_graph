...
In this section we will discuss particularities and the initialization of all current implementations.


Starting with the :code:`NodeSetS`, :code:`ns` this object can be constructed given a python an iterable of node identifiers or a set, which corresponds to its native object. To access the set, a property :code:`ns.nodes` returns a copy of it and :code:`ns.nodes_` the protected attribute of the class.
...
As our implementations generally rely on Pandas DataFrames, the node identifiers its better to be restricted to strings or integer, where the second case is preferable as pandas DataFrames are more efficient with node identifiers.
...
In case we have a non-discrete dataframe, the input is expected to be an iterable of the form :math:`(k*_{1}, .., k*_{n}, t_{s}, t_{f}, itype*, w*)` or :math:`(k*_{1}, .., k*_{n}, t_{s}, t_{f}, s*, f*, w*)`, following the same formalism as before with the difference that :code:`itype` (interval-type) signifies the closed interval bounding ('both', 'open', 'left', 'right') and :code:`s` and :code:`f` are booleans notifying if the start of the interval and the end are closed or not. If :code:`itype` or both :code:`s` and :code:`f` are ommited, a argument :code:`default_closed`, that by default 'both' in non-weighted cases and 'left' in weighted signifies, if the type of the interval is not defined.  
...
Additionally a parameter :code:`disjoint_intervals` signifies if the input contains only disjoint intervals, as this is important for the objects to function correctly. If the :code:`disjoint_intervals` is :code:`False` a function should be executed so that it makes them disjoint. Note here, that we make the assumption that links or nodes with same time-signature and same keys are considered duplicate entries and not different nodes. For the instantaneous case, this is given as an argument with the name :code:`no_duplicates`.
In the case of 'weighted' functions a :code:`merge_function` could be defined such that it summarizes same-weights. More-over an algebra for weights can be defined as a dictionary given in a property called :code:`operation_functions`. These functions are those of 'union', 'intersection', 'difference' and 'issuperset'.
In both cases if the objects are weighted the user should provide weights for all inputs.
All temporal-objects that end with a :code:`DF` have as a native object one in the subpackage :code:`stream_graph.base.dataframes`, discussed in the next section.
