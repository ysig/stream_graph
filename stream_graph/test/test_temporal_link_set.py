"""Test file for time set."""
from itertools import product
from pandas import DataFrame
from stream_graph import NodeSetS
from stream_graph import ITimeSetS
from stream_graph import TimeSetDF
from stream_graph import TemporalNodeSetB
from stream_graph import TemporalNodeSetDF
from stream_graph import ITemporalNodeSetDF
from stream_graph import TemporalLinkSetDF
from stream_graph import ITemporalLinkSetDF
from stream_graph.collections import TimeCollection
from stream_graph.collections import TimeGenerator
from stream_graph.exceptions import UnrecognizedTemporalLinkSet

def test_temporal_link_set_df():
    assert not bool(TemporalLinkSetDF([]))
    assert not bool(TemporalLinkSetDF())

    assert TemporalLinkSetDF().size == 0
    assert TemporalLinkSetDF([]).size == 0

    assert TemporalLinkSetDF().discrete is None

    for d in [False, True]:
        df = [(1, 2, 2, 3), (1, 2, 3, 5), (2, 1, 6, 8), (2, 1, 1, 3)]
        lsa = TemporalLinkSetDF(df, disjoint_intervals=False, discrete=d)

        assert bool(lsa)
        assert set(lsa) == {(1, 2, 2, 5), (2, 1, 6, 8), (2, 1, 1, 3)}

        assert lsa.m == 2
        assert lsa.size == 7 + 3*int(d)


        assert (1, 2, None) in lsa
        assert (1, None, 3.4) in lsa
        assert (5, None, None) not in lsa
        assert (1, 3, 3.4) not in lsa
        assert (1, 2, (3.4, 4)) in lsa

        assert lsa.duration_of((1, 2)) == 3 + int(d)
        assert lsa.duration_of((2, 1)) == 4 + 2*int(d)
        assert lsa.duration_of((2, 1)) == lsa.duration_of((1, 2), 'in')
        assert lsa.duration_of((1, 2), direction='both') == 6 + 2*int(d)
        assert lsa.duration_of((5, 1)) == 0
        assert dict(lsa.duration_of()) == {(1, 2): 3.0 + int(d), (2, 1): 4.0 + 2*int(d)}

        assert set(lsa.links_at(1)) == {(2, 1)}
        assert set(lsa.links_at((2, 2.5))) == {(1, 2), (2, 1)}
        assert set(lsa.links_at(10)) == set()
        
        tg = lsa.links_at()
        assert isinstance(tg, TimeGenerator)
        assert [(t, set(ns)) for t, ns in tg] == [(1, {(2, 1)}), (2, {(1, 2), (2, 1)}), (3, {(1, 2)}), (5, set()), (6, {(2, 1)}), (8, set())]
        assert not tg.instants
        assert not len([a for a in tg])

        assert set(lsa.times_of((1, 2))) == {(2, 5)}
        assert set(lsa.times_of((10, 3))) == set()
        assert {t: list(ns) for t, ns in lsa.times_of()} == {(1, 2): [(2, 5)], (2, 1): [(1, 3), (6, 8)]}

        assert lsa.m_at(2) == 2
        assert lsa.m_at(10) == 0

        df = [(1, 2, 1, 4), (1, 2, 6, 7), (2, 1, 2.5, 2.6)]
        lsb = TemporalLinkSetDF(df, discrete=d)
        assert set(lsb & lsa) == set(lsa & lsb)
        assert isinstance(lsa & lsb, TemporalLinkSetDF)
        assert set(lsa & lsb) == {(1, 2, 2, 4), (2, 1, 2.5, 2.6)}
        assert (lsa & lsb).size == 2.1 + 2*int(d)
        # incorrect for the discrete case as the intervals are not discrete

        assert isinstance(lsa | lsb, TemporalLinkSetDF)
        assert set((lsb | lsa)) == {(2, 1, 1, 3), (2, 1, 6, 8), (1, 2, 1, 5), (1, 2, 6, 7)}
        assert (lsb | lsa).size == 9 + 4*int(d)
        assert set(lsb | lsa) == set(lsa | lsb)

        assert set(lsb - lsa) == {(1, 2, 1, 2), (1, 2, 6, 7)}
        assert (lsb - lsa).size == 2 + 2*int(d)
        assert isinstance(lsb - lsa, TemporalLinkSetDF)
        assert set(lsa - lsb) == {(2, 1, 2.6, 3), (2, 1, 1, 2.5), (2, 1, 6, 8), (1, 2, 4, 5)}
        assert isinstance(lsa - lsb, TemporalLinkSetDF)
        assert (lsa - lsb).size == 4.9 + 4*int(d)

        assert lsa.issuperset(lsa & lsb)
        assert lsb.issuperset(lsa & lsb)
        assert (lsa | lsb).issuperset(lsa)
        assert (lsa | lsb).issuperset(lsb)
        assert lsa.issuperset(lsa - lsb)

        assert set(lsa.neighbors(1)) == {(2, 2, 5)}
        assert set(lsa.neighbors(1, 'in')) == {(2, 6, 8), (2, 1, 3)}
        assert set(lsa.neighbors(1, 'both')) == {(2, 6, 8), (2, 1, 5)}
        
        assert {n: list(ns) for n, ns in lsa.neighbors()} == {1: [(2, 2, 5)], 2: [(1, 1, 3), (1, 6, 8)]}

        na = lsa.neighbors_at(1)
        assert isinstance(na, TimeGenerator)
        assert [(t, set(ns)) for t, ns in na] == [(2, {2}), (5, set())]
        assert not tg.instants
        assert not len([a for a in na])
        
        assert set(lsa.neighbors_at(1, 2)) == {2}
        assert {i: set(nsa) for i, nsa in lsa.neighbors_at(None, 2, 'out')} == {1: {2}, 2: {1}}
        assert {i: [(t, set(ts)) for t, ts in nsa] for i, nsa in lsa.neighbors_at()} == {2: [(1, {1}), (3, set()), (6, {1}), (8, set())], 1: [(2, {2}), (5, set())]}

        assert {t: list(tc) for t, tc in lsa.degree_at()} == {2: [(1, 1), (3, 0), (6, 1), (8, 0)], 1: [(2, 1), (5, 0)]}

        na = lsa.degree_at(1)
        assert isinstance(na, TimeCollection)
        assert [(t, ns) for t, ns in na] == [(2, 1), (5, 0)]
        assert not na.instants
        assert len([a for a in na])
        
        assert lsa.degree_at(1, 2) == 1
        assert {i: n for i, n in lsa.degree_at(None, 2, 'out')} == {1: 1, 2: 1}
        assert {i: [(t, ts) for t, ts in nsa] for i, nsa in lsa.degree_at()} == {2: [(1, 1), (3, 0), (6, 1), (8, 0)], 1: [(2, 1), (5, 0)]}

        nsa = NodeSetS({1})
        nsb = NodeSetS({2})
        ts = TimeSetDF([(1, 4)], discrete=d)
        assert set(lsa.substream(nsa, nsb, ts)) == {(1, 2, 2, 4)}
        
        nsma = TemporalNodeSetDF([(1, 1, 4)], discrete=d)
        assert set(lsa.neighborhood(nsma, direction='out')) == {(2, 2, 4)}

        nsmb = TemporalNodeSetDF([(1, 1, 4), (2, 1, 2)], discrete=d)
        assert set(lsa.induced_substream(nsmb)) == {(1, 2, 2, 2), (2, 1, 1, 2)}


        try:
            lsa | 1
        except UnrecognizedTemporalLinkSet:
            pass

        try:
            lsa & 1
        except UnrecognizedTemporalLinkSet:
            pass

        try:
            lsa - 1
        except UnrecognizedTemporalLinkSet:
            pass

        df = [(1, 2, 2, 10), (2, 3, 4, 16), (1, 3, 6, 12), (3, 4, 8, 16), (2, 4, 13, 17)]
        assert TemporalLinkSetDF(df, discrete=d).get_maximal_cliques() == {(frozenset({2, 4}), (13, 17)),
                                                                           (frozenset({3, 4}), (8, 16)),
                                                                           (frozenset({2, 3, 4}), (13, 16)),
                                                                           (frozenset({2, 3}), (4, 16)),
                                                                           (frozenset({1, 2, 3}), (6, 10)),
                                                                           (frozenset({1, 3}), (6, 12)),
                                                                           (frozenset({1, 2}), (2, 10))}


    df = [(1, 2, 2, 3), (1, 2, 3, 5), (2, 1, 6, 8), (2, 1, 1, 3)]
    lsa = TemporalLinkSetDF(df, disjoint_intervals=False, weighted=True)

    assert bool(lsa)
    assert set(lsa) == {(1, 2, 2, 5, 1), (2, 1, 6, 8, 1), (2, 1, 1, 3, 1)}

    assert lsa.m == 2
    assert lsa.size == 7


    assert (1, 2, None) in lsa
    assert (1, None, 3.4) in lsa
    assert (5, None, None) not in lsa
    assert (1, 3, 3.4) not in lsa
    assert (1, 2, (3.4, 4)) in lsa

    assert lsa.duration_of((1, 2)) == 3
    assert lsa.duration_of((2, 1)) == 4
    assert lsa.duration_of((2, 1)) == lsa.duration_of((1, 2), 'in')
    assert lsa.duration_of((1, 2), direction='both') == 6
    assert lsa.duration_of((5, 1)) == 0
    assert dict(lsa.duration_of()) == {(1, 2): 3.0, (2, 1): 4.0}

    assert set(lsa.links_at(1)) == {(2, 1, 1)}
    assert set(lsa.links_at((2, 2.5))) == {(1, 2, 1), (2, 1, 1)}
    assert set(lsa.links_at(10)) == set()
    
    tg = lsa.links_at()
    assert isinstance(tg, TimeGenerator)
    assert [(t, set(ns)) for t, ns in tg] == [(1, {(2, 1, 1)}), (2, {(1, 2, 1), (2, 1, 1)}), (3, {(1, 2, 1)}), (5, set()), (6, {(2, 1, 1)}), (8, set())]
    assert not tg.instants
    assert not len([a for a in tg])

    assert set(lsa.times_of((1, 2))) == {(2, 5)}
    assert set(lsa.times_of((10, 3))) == set()
    assert {t: list(ns) for t, ns in lsa.times_of()} == {(1, 2): [(2, 5)], (2, 1): [(1, 3), (6, 8)]}

    assert lsa.m_at(2) == 2
    assert lsa.m_at(10) == 0

    df = [(1, 2, 1, 4), (1, 2, 6, 7), (2, 1, 2.5, 2.6)]
    lsb = TemporalLinkSetDF(df, weighted=True)
    assert set(lsb & lsa) == set(lsa & lsb)
    assert isinstance(lsa & lsb, TemporalLinkSetDF)
    assert set(lsa & lsb) == {(1, 2, 2, 4, 2.0), (2, 1, 2.5, 2.6, 2.0)}
    assert (lsa & lsb).size == 2.1
    # incorrect for the discrete case as the intervals are not discrete

    assert isinstance(lsa | lsb, TemporalLinkSetDF)
    assert set((lsb | lsa)) == {(2, 1, 2.5, 2.6, 2.0), (2, 1, 1.0, 2.5, 1.0), (1, 2, 6.0, 7.0, 1.0), (1, 2, 4.0, 5.0, 1.0), (2, 1, 6.0, 8.0, 1.0), (2, 1, 2.6, 3.0, 1.0), (1, 2, 2.0, 4.0, 2.0), (1, 2, 1.0, 2.0, 1.0)}
    assert (lsb | lsa).size == 9
    assert set(lsb | lsa) == set(lsa | lsb)

    assert set(lsb - lsa) == {(1, 2, 6.0, 7.0, 1.0), (1, 2, 1.0, 2.0, 1.0)}
    assert (lsb - lsa).size == 2
    assert isinstance(lsb - lsa, TemporalLinkSetDF)
    assert set(lsa - lsb) == {(1, 2, 4.0, 5.0, 1.0), (2, 1, 1.0, 2.5, 1.0), (2, 1, 6.0, 8.0, 1.0), (2, 1, 2.6, 3.0, 1.0)}
    assert isinstance(lsa - lsb, TemporalLinkSetDF)
    assert (lsa - lsb).size == 4.9

    assert lsa.issuperset(lsa & lsb)
    assert lsb.issuperset(lsa & lsb)
    assert (lsa | lsb).issuperset(lsa)
    assert (lsa | lsb).issuperset(lsb)
    assert lsa.issuperset(lsa - lsb)

    assert set(lsa.neighbors(1)) == {(2, 2, 5)}
    assert set(lsa.neighbors(1, 'in')) == {(2, 6, 8), (2, 1, 3)}
    assert set(lsa.neighbors(1, 'both')) == {(2, 6, 8), (2, 1, 5)}
    
    assert {n: list(ns) for n, ns in lsa.neighbors()} == {1: [(2, 2, 5)], 2: [(1, 1, 3), (1, 6, 8)]}

    na = lsa.neighbors_at(1)
    assert isinstance(na, TimeGenerator)
    assert [(t, set(ns)) for t, ns in na] == [(2, {2}), (5, set())]
    assert not tg.instants
    assert not len([a for a in na])
    
    assert set(lsa.neighbors_at(1, 2)) == {2}
    assert {i: set(nsa) for i, nsa in lsa.neighbors_at(None, 2, 'out')} == {1: {2}, 2: {1}}
    assert {i: [(t, set(ts)) for t, ts in nsa] for i, nsa in lsa.neighbors_at()} == {2: [(1, {1}), (3, set()), (6, {1}), (8, set())], 1: [(2, {2}), (5, set())]}

    for w in [False, True]:
        assert {t: list(tc) for t, tc in lsa.degree_at(weights=w)} == {2: [(1, 1), (3, 0), (6, 1), (8, 0)], 1: [(2, 1), (5, 0)]}

        na = lsa.degree_at(1, weights=w)
        assert isinstance(na, TimeCollection)
        assert [(t, ns) for t, ns in na] == [(2, 1), (5, 0)]
        assert not na.instants
        assert len([a for a in na])
        
        assert lsa.degree_at(1, 2, weights=w) == 1
        assert {i: n for i, n in lsa.degree_at(None, 2, 'out', weights=w)} == {1: 1, 2: 1}
        assert {i: [(t, ts) for t, ts in nsa] for i, nsa in lsa.degree_at(weights=w)} == {2: [(1, 1), (3, 0), (6, 1), (8, 0)], 1: [(2, 1), (5, 0)]}

    nsa = NodeSetS({1})
    nsb = NodeSetS({2})
    ts = TimeSetDF([(1, 4)])
    assert set(lsa.substream(nsa, nsb, ts)) == {(1, 2, 2, 4, 1.0)}
    
    nsma = TemporalNodeSetDF([(1, 1, 4)])
    assert set(lsa.neighborhood(nsma, direction='out')) == {(2, 2, 4)}

    nsmb = TemporalNodeSetDF([(1, 1, 4), (2, 1, 2)])
    assert set(lsa.induced_substream(nsmb)) == {(1, 2, 2, 2, 1.0), (2, 1, 1, 2, 1.0)}


    try:
        lsa | 1
    except UnrecognizedTemporalLinkSet:
        pass

    try:
        lsa & 1
    except UnrecognizedTemporalLinkSet:
        pass

    try:
        lsa - 1
    except UnrecognizedTemporalLinkSet:
        pass

    df = [(1, 2, 2, 10), (2, 3, 4, 16), (1, 3, 6, 12), (3, 4, 8, 16), (2, 4, 13, 17)]
    assert TemporalLinkSetDF(df).get_maximal_cliques() == {(frozenset({2, 4}), (13, 17)),
                                                           (frozenset({3, 4}), (8, 16)),
                                                           (frozenset({2, 3, 4}), (13, 16)),
                                                           (frozenset({2, 3}), (4, 16)),
                                                           (frozenset({1, 2, 3}), (6, 10)),
                                                           (frozenset({1, 3}), (6, 12)),
                                                           (frozenset({1, 2}), (2, 10))}


def test_itemporal_link_set_df():
    assert ITemporalLinkSetDF().size == 0
    assert ITemporalLinkSetDF([]).size == 0
    assert ITemporalLinkSetDF([]).discrete is None

    for d in [False, True]:
        df = [(1, 2, 2), (1, 2, 3), (2, 1, 6), (2, 1, 3), (2, 1, 3)]
        lsa = ITemporalLinkSetDF(df, no_duplicates=False, discrete=d)

        assert bool(lsa)
        assert set(lsa) == {(1, 2, 2), (1, 2, 3), (2, 1, 6), (2, 1, 3)}

        assert lsa.m == 2
        assert lsa.size == 4*int(d)

        assert (1, 2, None) in lsa
        assert (1, None, 2) in lsa
        assert (5, None, None) not in lsa
        assert (1, 3, 1) not in lsa
        assert (1, 2, 2) in lsa

        assert lsa.duration_of((1, 2)) == 2*int(d)
        assert lsa.duration_of((5, 1)) == 0
        assert dict(lsa.duration_of()) == {(1, 2): 2*int(d), (2, 1): 2*int(d)}

        assert set(lsa.links_at(6)) == {(2, 1)}
        assert set(lsa.links_at(3)) == {(1, 2), (2, 1)}
        assert set(lsa.links_at(10)) == set()

        la = lsa.links_at()
        assert isinstance(la, TimeGenerator)
        assert [(t, list(ls)) for t, ls in la] == [(2, [(1, 2)]), (3, [(1, 2), (2, 1)]), (6, [(2, 1)])]
        assert la.instants
        assert not len([a for a in la])

        assert set(lsa.times_of((1, 2))) == {2, 3}
        assert set(lsa.times_of((10, 3))) == set()
        assert {l: set(s) for l, s in lsa.times_of()} == {(1, 2): {2, 3}, (2, 1): {3, 6}}

        assert lsa.m_at(3) == 2
        assert lsa.m_at(10) == 0
        assert list(lsa.m_at()) == [(2, 1), (3, 2), (6, 1)]

        df = [(1, 2, 2), (1, 2, 4), (2, 1, 7), (2, 1, 3)]
        lsb = ITemporalLinkSetDF(df, discrete=d)
        assert set(lsb & lsa) == set(lsa & lsb)
        assert isinstance(lsa & lsb, ITemporalLinkSetDF)
        assert set(lsa & lsb) == {(1, 2, 2), (2, 1, 3)}

        assert isinstance(lsa | lsb, ITemporalLinkSetDF)
        assert set(lsb | lsa) == set(lsa | lsb)
        assert set((lsb | lsa)) == {(2, 1, 3), (2, 1, 7), (2, 1, 6), (1, 2, 2), (1, 2, 3), (1, 2, 4)}

        assert set(lsb - lsa) == {(1, 2, 4), (2, 1, 7)}
        assert isinstance(lsb - lsa, ITemporalLinkSetDF)
        assert set(lsa - lsb) == {(1, 2, 3), (2, 1, 6)}
        assert isinstance(lsa - lsb, ITemporalLinkSetDF)

        assert lsa.issuperset(lsa & lsb)
        assert lsb.issuperset(lsa & lsb)
        assert (lsa | lsb).issuperset(lsa)
        assert (lsa | lsb).issuperset(lsb)
        assert lsa.issuperset(lsa - lsb)

        assert set(lsa.neighbors(1)) == {(2, 2), (2, 3)}
        assert isinstance(lsa.neighbors(1), ITemporalNodeSetDF)
        assert set(lsa.neighbors(1, 'in')) == {(2, 6), (2, 3)}
        assert set(lsa.neighbors(1, 'both')) == {(2, 2), (2, 3), (2, 6)}

        assert {n: list(ns) for n, ns in lsa.neighbors()} == {1: [(2, 2), (2, 3)], 2: [(1, 3), (1, 6)]}

        la = lsa.neighbors_at(1)
        assert isinstance(la, TimeGenerator)
        assert [(t, set(ts)) for t, ts in la] == [(2, {2}), (3, {2})]
        assert la.instants
        assert not len([a for a in la])

        assert set(lsa.neighbors_at(1, 2)) == {2}
        assert {i: set(nsa) for i, nsa in lsa.neighbors_at(None, 2, 'out')} == {1: {2}}
        assert {i: [(t, set(ts)) for t, ts in nsa] for i, nsa in lsa.neighbors_at()} == {1: [(2, {2}), (3, {2})], 2: [(3, {1}), (6, {1})]}
        la = lsa.degree_at(1)
        assert isinstance(la, TimeGenerator)
        assert list(la) == [(2, 1), (3, 1)]
        assert la.instants
        assert len([a for a in la])

        assert lsa.degree_at(1, 2) == 1
        assert {i: n for i, n in lsa.degree_at(None, 2, 'out')} == {1: 1}
        assert {i: [(t, ts) for t, ts in nsa] for i, nsa in lsa.degree_at()} == {1: [(2, 1), (3, 1)], 2: [(3, 1), (6, 1)]}

        nsa = NodeSetS({1})
        nsb = NodeSetS({2})
        ts = ITimeSetS([(2)], discrete=d)
        assert set(lsa.substream(nsa, nsb, ts)) == {(1, 2, 2)}
        
        nsma = ITemporalNodeSetDF([(1, 2), (1, 3)], discrete=d)
        assert set(lsa.neighborhood(nsma, direction='out')) == {(2, 2), (2, 3)}

        nsmb = ITemporalNodeSetDF([(1, 3), (2, 3)], discrete=d)
        assert set(lsa.induced_substream(nsmb)) == {(1, 2, 3), (2, 1, 3)}

        try:
            lsa | 1
        except UnrecognizedTemporalLinkSet:
            pass

        try:
            lsa & 1
        except UnrecognizedTemporalLinkSet:
            pass

        try:
            lsa - 1
        except UnrecognizedTemporalLinkSet:
            pass

        df = [(1, 2, 4), (1, 2, 8), (2, 3, 4), (1, 3, 6), (3, 4, 2), (2, 4, 3)]
        assert ITemporalLinkSetDF(df, discrete=d).get_maximal_cliques(delta=3) == {(frozenset({3, 4}), (3.5, 6.5)),
                                                                       (frozenset({1, 2, 3}), (7.5, 8.5)), 
                                                                       (frozenset({1, 2}), (5.5, 8.5)),
                                                                       (frozenset({2, 4}), (4.5, 7.5)),
                                                                       (frozenset({1, 2}), (9.5, 12.5)),
                                                                       (frozenset({2, 3, 4}), (5.5, 6.5)),
                                                                       (frozenset({2, 3}), (5.5, 8.5)),
                                                                       (frozenset({1, 3}), (7.5, 10.5))}

        df = [(1, 2, 2), (1, 2, 3), (2, 3, 2), (1, 3, 4), (3, 4, 3), (2, 4, 5)]    
        assert ITemporalLinkSetDF(df, discrete=d).centrality(3, direction='both') == [(2, 0.0), (3, 1.0)]

    df = [(1, 2, 2), (1, 2, 3), (2, 1, 6), (2, 1, 3), (2, 1, 3)]
    lsa = ITemporalLinkSetDF(df, no_duplicates=False, weighted=True)

    assert bool(lsa)
    assert set(lsa) == {(1, 2, 2, 1), (1, 2, 3, 1), (2, 1, 6, 1), (2, 1, 3, 1)}

    assert lsa.m == 2
    assert lsa.size == 0

    assert (1, 2, None) in lsa
    assert (1, None, 2) in lsa
    assert (5, None, None) not in lsa
    assert (1, 3, 1) not in lsa
    assert (1, 2, 2) in lsa

    assert lsa.duration_of((1, 2)) == 0
    assert lsa.duration_of((5, 1)) == 0
    assert dict(lsa.duration_of()) == {(1, 2): 0, (2, 1): 0}

    assert set(lsa.links_at(6)) == {(2, 1, 1.0)}
    assert set(lsa.links_at(3)) == {(1, 2, 1.0), (2, 1, 1.0)}
    assert set(lsa.links_at(10)) == set()

    la = lsa.links_at()
    assert isinstance(la, TimeGenerator)
    assert [(t, list(ls)) for t, ls in la] == [(2, [(1, 2, 1.0)]), (3, [(1, 2, 1.0), (2, 1, 1.0)]), (6, [(2, 1, 1.0)])]
    assert la.instants
    assert not len([a for a in la])

    assert set(lsa.times_of((1, 2))) == {2, 3}
    assert set(lsa.times_of((10, 3))) == set()
    assert {l: set(s) for l, s in lsa.times_of()} == {(1, 2): {2, 3}, (2, 1): {3, 6}}

    df = [(1, 2, 2), (1, 2, 4), (2, 1, 7), (2, 1, 3)]
    lsb = ITemporalLinkSetDF(df, weighted=True)
    assert set(lsb & lsa) == set(lsa & lsb)
    assert isinstance(lsa & lsb, ITemporalLinkSetDF)
    assert set(lsa & lsb) == {(2, 1, 3, 2), (1, 2, 2, 2)}

    assert isinstance(lsa | lsb, ITemporalLinkSetDF)
    assert set(lsb | lsa) == set(lsa | lsb)
    assert set((lsb | lsa)) == {(1, 2, 3, 1), (1, 2, 4, 1), (2, 1, 7, 1), (2, 1, 3, 2), (1, 2, 2, 2), (2, 1, 6, 1)}

    assert set(lsb - lsa) == {(1, 2, 4, 1), (2, 1, 7, 1)}
    assert isinstance(lsb - lsa, ITemporalLinkSetDF)
    assert set(lsa - lsb) == {(1, 2, 3, 1), (2, 1, 6, 1)}
    assert isinstance(lsa - lsb, ITemporalLinkSetDF)

    assert lsa.issuperset(lsa & lsb)
    assert lsb.issuperset(lsa & lsb)
    assert (lsa | lsb).issuperset(lsa)
    assert (lsa | lsb).issuperset(lsb)
    assert lsa.issuperset(lsa - lsb)

    assert set(lsa.neighbors(1)) == {(2, 2), (2, 3)}
    assert isinstance(lsa.neighbors(1), ITemporalNodeSetDF)
    assert set(lsa.neighbors(1, 'in')) == {(2, 6), (2, 3)}
    assert set(lsa.neighbors(1, 'both')) == {(2, 2), (2, 3), (2, 6)}

    assert {n: list(ns) for n, ns in lsa.neighbors()} == {1: [(2, 2), (2, 3)], 2: [(1, 3), (1, 6)]}

    la = lsa.neighbors_at(1)
    assert isinstance(la, TimeGenerator)
    assert [(t, set(ts)) for t, ts in la] == [(2, {2}), (3, {2})]
    assert la.instants
    assert not len([a for a in la])

    assert set(lsa.neighbors_at(1, 2)) == {2}
    assert {i: set(nsa) for i, nsa in lsa.neighbors_at(None, 2, 'out')} == {1: {2}}
    assert {i: [(t, set(ts)) for t, ts in nsa] for i, nsa in lsa.neighbors_at()} == {1: [(2, {2}), (3, {2})], 2: [(3, {1}), (6, {1})]}

    for w in [False, True]:
        assert lsa.m_at(3, weights=w) == 2
        assert lsa.m_at(10, weights=w) == 0
        assert list(lsa.m_at(weights=w)) == [(2, 1), (3, 2), (6, 1)]

        la = lsa.degree_at(1, weights=w)
        assert isinstance(la, TimeGenerator)
        assert list(la) == [(2, 1), (3, 1)]
        assert la.instants
        assert len([a for a in la])

        assert lsa.degree_at(1, 2, weights=w) == 1
        assert {i: n for i, n in lsa.degree_at(None, 2, 'out', weights=w)} == {1: 1}
        assert {i: [(t, ts) for t, ts in nsa] for i, nsa in lsa.degree_at(weights=w)} == {1: [(2, 1), (3, 1)], 2: [(3, 1), (6, 1)]}

    nsa = NodeSetS({1})
    nsb = NodeSetS({2})
    ts = ITimeSetS([(2)])
    assert set(lsa.substream(nsa, nsb, ts)) == {(1, 2, 2, 1)}
    
    nsma = ITemporalNodeSetDF([(1, 2), (1, 3)])
    assert set(lsa.neighborhood(nsma, direction='out')) == {(2, 2), (2, 3)}

    nsmb = ITemporalNodeSetDF([(1, 3), (2, 3)])
    assert set(lsa.induced_substream(nsmb)) == {(1, 2, 3, 1), (2, 1, 3, 1)}

    try:
        lsa | 1
    except UnrecognizedTemporalLinkSet:
        pass

    try:
        lsa & 1
    except UnrecognizedTemporalLinkSet:
        pass

    try:
        lsa - 1
    except UnrecognizedTemporalLinkSet:
        pass

    df = [(1, 2, 4), (1, 2, 8), (2, 3, 4), (1, 3, 6), (3, 4, 2), (2, 4, 3)]
    assert ITemporalLinkSetDF(df, weighted=True).get_maximal_cliques(delta=3) == {(frozenset({3, 4}), (3.5, 6.5)),
                                                                                  (frozenset({1, 2, 3}), (7.5, 8.5)), 
                                                                                  (frozenset({1, 2}), (5.5, 8.5)),
                                                                                  (frozenset({2, 4}), (4.5, 7.5)),
                                                                                  (frozenset({1, 2}), (9.5, 12.5)),
                                                                                  (frozenset({2, 3, 4}), (5.5, 6.5)),
                                                                                  (frozenset({2, 3}), (5.5, 8.5)),
                                                                                  (frozenset({1, 3}), (7.5, 10.5))}
    
    df = [(1, 2, 2), (1, 2, 3), (2, 3, 2), (1, 3, 4), (3, 4, 3), (2, 4, 5)]    
    assert ITemporalLinkSetDF(df, weighted=True).centrality(3, direction='both') == [(2, 0.0), (3, 1.0)]

if __name__ == "__main__":
    test_temporal_link_set_df()
    test_itemporal_link_set_df()
